...
!
    DO k=1,max_points
      ...
      beta = 0.5d0*rho/pr
      temp = u1*u1 + u2*u2
      pointd%q(1, k) = rhod/rho + (2.5d0/beta-temp)*betad - beta*(2*u1*u1d+2*u2*u2d)
  ...
  SUBROUTINE EVAL_Q_DERIVATIVES_D()
    ...
    DO i=1,local_points
      ...
      DO k=1,point%nbhs(i)
        nbh = point%conn(i, k)
        DO r=1,4
          IF (point%q(r, nbh) .GT. point%qm(1, r, i)) point%qm(1, r, i) = point%q(r, nbh)
          IF (point%q(r, nbh) .LT. point%qm(2, r, i)) point%qm(2, r, i) = point%q(r, nbh)
        END DO
        x_k = point%x(nbh)
        y_k = point%y(nbh)
        delx = x_k - x_i
        ...
        sum_delx_delqd = sum_delx_delqd + weights*delx*(pointd%q(:, nbh)-pointd%q(:, i))
        sum_delx_delq = sum_delx_delq + weights*delx*(point%q(:, nbh)-point%q(:, i))
        sum_dely_delqd = sum_dely_delqd + weights*dely*(pointd%q(:, nbh)-pointd%q(:, i))
        sum_dely_delq = sum_dely_delq + weights*dely*(point%q(:, nbh)-point%q(:, i))
      END DO
      det = sum_delx_sqr*sum_dely_sqr - sum_delx_dely*sum_delx_dely
      one_by_det = 1.0d0/det
      pointd%dq(1, :, i) = one_by_det*(sum_dely_sqr*sum_delx_delqd-sum_delx_dely*sum_dely_delqd)
      point%dq(1, :, i) = (sum_delx_delq*sum_dely_sqr-sum_dely_delq*sum_delx_dely)*one_by_det
      pointd%dq(2, :, i) = one_by_det*(sum_delx_sqr*sum_dely_delqd-sum_delx_dely*sum_delx_delqd)
      point%dq(2, :, i) = (sum_dely_delq*sum_delx_sqr-sum_delx_delq*sum_delx_dely)*one_by_det
    END DO
...
! local variables
    ...
    REAL*8 :: sum_delx_del_qxd(4), sum_delx_del_qyd(4), sum_dely_del_qxd(4), sum_dely_del_qyd(4)
    REAL*8 :: det, delq, temp
    REAL*8 :: one_by_det
    INTRINSIC DSQRT
    REAL*8 :: arg1
    DO i=1,local_points
      ...
      DO k=1,point%nbhs(i)
        ...
        sum_dely_sqr = sum_dely_sqr + dely*dely*weights
        sum_delx_dely = sum_delx_dely + delx*dely*weights
        sum_delx_del_qxd = sum_delx_del_qxd + weights*delx*(pointd%dq(1, :, nbh)-pointd%dq(1, :, i))
        sum_delx_del_qx = sum_delx_del_qx + weights*delx*(point%dq(1, :, nbh)-point%dq(1, :, i))
        sum_delx_del_qyd = sum_delx_del_qyd + weights*delx*(pointd%dq(2, :, nbh)-pointd%dq(2, :, i))
        sum_delx_del_qy = sum_delx_del_qy + weights*delx*(point%dq(2, :, nbh)-point%dq(2, :, i))
        sum_dely_del_qxd = sum_dely_del_qxd + weights*dely*(pointd%dq(1, :, nbh)-pointd%dq(1, :, i))
        sum_dely_del_qx = sum_dely_del_qx + weights*dely*(point%dq(1, :, nbh)-point%dq(1, :, i))
        sum_dely_del_qyd = sum_dely_del_qyd + weights*dely*(pointd%dq(2, :, nbh)-pointd%dq(2, :, i))
        sum_dely_del_qy = sum_dely_del_qy + weights*dely*(point%dq(2, :, nbh)-point%dq(2, :, i))
      END DO
      det = sum_delx_sqr*sum_dely_sqr - sum_delx_dely*sum_delx_dely
      one_by_det = 1.0d0/det
      pointd%ddq(1, :, i) = one_by_det*(sum_dely_sqr*sum_delx_del_qxd-sum_delx_dely*sum_dely_del_qxd)
      point%ddq(1, :, i) = (sum_delx_del_qx*sum_dely_sqr-sum_dely_del_qx*sum_delx_dely)*one_by_det
      pointd%ddq(2, :, i) = one_by_det*(sum_dely_sqr*sum_delx_del_qyd-sum_delx_dely*sum_dely_del_qyd)
      point%ddq(2, :, i) = (sum_delx_del_qy*sum_dely_sqr-sum_dely_del_qy*sum_delx_dely)*one_by_det
      pointd%ddq(3, :, i) = one_by_det*(sum_delx_sqr*sum_dely_del_qyd-sum_delx_dely*sum_delx_del_qyd)
      point%ddq(3, :, i) = (sum_dely_del_qy*sum_delx_sqr-sum_delx_del_qy*sum_delx_dely)*one_by_det
    END DO
...
!                point.temp:in
  SUBROUTINE EVAL_Q_INNER_LOOP_D()
    ...
    REAL*8 :: sum_delx_delq1d, sum_delx_delq2d, sum_delx_delq3d, sum_delx_delq4d
    REAL*8 :: sum_dely_delq1, sum_dely_delq2, sum_dely_delq3, sum_dely_delq4
    REAL*8 :: sum_dely_delq1d, sum_dely_delq2d, sum_dely_delq3d, sum_dely_delq4d
    REAL*8 :: q1, q2, q3, q4
    ...
    DO i=1,local_points
      ...
      DO k=1,point%nbhs(i)
        ...
        sum_dely_sqr = sum_dely_sqr + dely*dely*weights
        sum_delx_dely = sum_delx_dely + delx*dely*weights
        temp0 = 2.0d0*delx*dely
        temp1d = q1d - 0.5d0*(delx*pointd%dq(1, 1, i)+dely*pointd%dq(2, 1, i)) + (delx**2*pointd%ddq(1, 1, i)+temp0*pointd%ddq(2, 1, i)+dely**2*pointd%ddq(3, 1, i))/12.0d0
        temp1 = q1 - 0.5d0*(delx*point%dq(1, 1, i)+dely*point%dq(2, 1, i)) + (delx*delx*point%ddq(1, 1, i)+temp0*point%ddq(2, 1, i)+dely*dely*point%ddq(3, 1, i))/12.0d0
        temp0 = 2.0d0*delx*dely
        temp2d = pointd%q(1, nbh) - 0.5d0*(delx*pointd%dq(1, 1, nbh)+dely*pointd%dq(2, 1, nbh)) + (delx**2*pointd%ddq(1, 1, nbh)+temp0*pointd%ddq(2, 1, nbh)+dely**2*pointd%ddq(3, 1, nbh))/12.0d0
        temp2 = point%q(1, nbh) - 0.5d0*(delx*point%dq(1, 1, nbh)+dely*point%dq(2, 1, nbh)) + (delx*delx*point%ddq(1, 1, nbh)+temp0*point%ddq(2, 1, nbh)+dely*dely*point%ddq(3, 1, nbh))/12.0d0
        ...
        temp0 = 2.0d0*delx*dely
        temp1d = q2d - 0.5d0*(delx*pointd%dq(1, 2, i)+dely*pointd%dq(2, 2, i)) + (delx**2*pointd%ddq(1, 2, i)+temp0*pointd%ddq(2, 2, i)+dely**2*pointd%ddq(3, 2, i))/12.0d0
        temp1 = q2 - 0.5d0*(delx*point%dq(1, 2, i)+dely*point%dq(2, 2, i)) + (delx*delx*point%ddq(1, 2, i)+temp0*point%ddq(2, 2, i)+dely*dely*point%ddq(3, 2, i))/12.0d0
        temp0 = 2.0d0*delx*dely
        temp2d = pointd%q(2, nbh) - 0.5d0*(delx*pointd%dq(1, 2, nbh)+dely*pointd%dq(2, 2, nbh)) + (delx**2*pointd%ddq(1, 2, nbh)+temp0*pointd%ddq(2, 2, nbh)+dely**2*pointd%ddq(3, 2, nbh))/12.0d0
        temp2 = point%q(2, nbh) - 0.5d0*(delx*point%dq(1, 2, nbh)+dely*point%dq(2, 2, nbh)) + (delx*delx*point%ddq(1, 2, nbh)+temp0*point%ddq(2, 2, nbh)+dely*dely*point%ddq(3, 2, nbh))/12.0d0
        ...
        temp0 = 2.0d0*delx*dely
        temp1d = q3d - 0.5d0*(delx*pointd%dq(1, 3, i)+dely*pointd%dq(2, 3, i)) + (delx**2*pointd%ddq(1, 3, i)+temp0*pointd%ddq(2, 3, i)+dely**2*pointd%ddq(3, 3, i))/12.0d0
        temp1 = q3 - 0.5d0*(delx*point%dq(1, 3, i)+dely*point%dq(2, 3, i)) + (delx*delx*point%ddq(1, 3, i)+temp0*point%ddq(2, 3, i)+dely*dely*point%ddq(3, 3, i))/12.0d0
        temp0 = 2.0d0*delx*dely
        temp2d = pointd%q(3, nbh) - 0.5d0*(delx*pointd%dq(1, 3, nbh)+dely*pointd%dq(2, 3, nbh)) + (delx**2*pointd%ddq(1, 3, nbh)+temp0*pointd%ddq(2, 3, nbh)+dely**2*pointd%ddq(3, 3, nbh))/12.0d0
        temp2 = point%q(3, nbh) - 0.5d0*(delx*point%dq(1, 3, nbh)+dely*point%dq(2, 3, nbh)) + (delx*delx*point%ddq(1, 3, nbh)+temp0*point%ddq(2, 3, nbh)+dely*dely*point%ddq(3, 3, nbh))/12.0d0
        ...
        temp0 = 2.0d0*delx*dely
        temp1d = q4d - 0.5d0*(delx*pointd%dq(1, 4, i)+dely*pointd%dq(2, 4, i)) + (delx**2*pointd%ddq(1, 4, i)+temp0*pointd%ddq(2, 4, i)+dely**2*pointd%ddq(3, 4, i))/12.0d0
        temp1 = q4 - 0.5d0*(delx*point%dq(1, 4, i)+dely*point%dq(2, 4, i)) + (delx*delx*point%ddq(1, 4, i)+temp0*point%ddq(2, 4, i)+dely*dely*point%ddq(3, 4, i))/12.0d0
        temp0 = 2.0d0*delx*dely
        temp2d = pointd%q(4, nbh) - 0.5d0*(delx*pointd%dq(1, 4, nbh)+dely*pointd%dq(2, 4, nbh)) + (delx**2*pointd%ddq(1, 4, nbh)+temp0*pointd%ddq(2, 4, nbh)+dely**2*pointd%ddq(3, 4, nbh))/12.0d0
        temp2 = point%q(4, nbh) - 0.5d0*(delx*point%dq(1, 4, nbh)+dely*point%dq(2, 4, nbh)) + (delx*delx*point%ddq(1, 4, nbh)+temp0*point%ddq(2, 4, nbh)+dely*dely*point%ddq(3, 4, nbh))/12.0d0
      ...
      det = sum_delx_sqr*sum_dely_sqr - sum_delx_dely*sum_delx_dely
      one_by_det = 1.0d0/det
      pointd%temp(1, 1, i) = one_by_det*(sum_dely_sqr*sum_delx_delq1d-sum_delx_dely*sum_dely_delq1d)
      point%temp(1, 1, i) = one_by_det*(sum_delx_delq1*sum_dely_sqr-sum_dely_delq1*sum_delx_dely)
      pointd%temp(1, 2, i) = one_by_det*(sum_dely_sqr*sum_delx_delq2d-sum_delx_dely*sum_dely_delq2d)
      point%temp(1, 2, i) = one_by_det*(sum_delx_delq2*sum_dely_sqr-sum_dely_delq2*sum_delx_dely)
      pointd%temp(1, 3, i) = one_by_det*(sum_dely_sqr*sum_delx_delq3d-sum_delx_dely*sum_dely_delq3d)
      point%temp(1, 3, i) = one_by_det*(sum_delx_delq3*sum_dely_sqr-sum_dely_delq3*sum_delx_dely)
      pointd%temp(1, 4, i) = one_by_det*(sum_dely_sqr*sum_delx_delq4d-sum_delx_dely*sum_dely_delq4d)
      point%temp(1, 4, i) = one_by_det*(sum_delx_delq4*sum_dely_sqr-sum_dely_delq4*sum_delx_dely)
      pointd%temp(2, 1, i) = one_by_det*(sum_delx_sqr*sum_dely_delq1d-sum_delx_dely*sum_delx_delq1d)
      point%temp(2, 1, i) = one_by_det*(sum_dely_delq1*sum_delx_sqr-sum_delx_delq1*sum_delx_dely)
      pointd%temp(2, 2, i) = one_by_det*(sum_delx_sqr*sum_dely_delq2d-sum_delx_dely*sum_delx_delq2d)
      point%temp(2, 2, i) = one_by_det*(sum_dely_delq2*sum_delx_sqr-sum_delx_delq2*sum_delx_dely)
      pointd%temp(2, 3, i) = one_by_det*(sum_delx_sqr*sum_dely_delq3d-sum_delx_dely*sum_delx_delq3d)
      point%temp(2, 3, i) = one_by_det*(sum_dely_delq3*sum_delx_sqr-sum_delx_delq3*sum_delx_dely)
      pointd%temp(2, 4, i) = one_by_det*(sum_delx_sqr*sum_dely_delq4d-sum_delx_dely*sum_delx_delq4d)
...
!   with respect to varying inputs: qtilde
  SUBROUTINE QTILDE_TO_PRIMITIVE_D(qtilde, qtilded, u1, u1d, u2, u2d, rho, rhod, pr, prd)
    IMPLICIT NONE
