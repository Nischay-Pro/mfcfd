...
!       No of shapes
  INTEGER :: shapes
  NAMELIST /input_parameters/ shapes, cfl, max_iters, vl_const, power, restart_solution, solution_accuracy, format_file, nsave, interior_points_normal_flag, tscheme, mach, aoa, inner_iterations
...
CONTAINS
  SUBROUTINE READNML()
    ...
    WRITE(*, *) 
    IF (solution_accuracy .EQ. 'second') THEN
      f_o_flag = 1.0d0
    ELSE IF (solution_accuracy .EQ. 'first') THEN
      f_o_flag = 0.0d0
    END IF
    IF (restart_solution .EQ. 'no') THEN
      solution_restart = 0
    ELSE IF (restart_solution .EQ. 'yes') THEN
      solution_restart = 1
    END IF
    IF (tscheme .EQ. 'first') THEN
      rks = 1
      euler = 2.0d0
    ELSE IF (tscheme .EQ. 'ssprk43') THEN
      rks = 4
      euler = 1.0d0
    END IF
    IF (format_file .EQ. 'legacy') THEN
      file_format = 1
    ELSE IF (format_file .EQ. 'quadtree') THEN
      file_format = 2
