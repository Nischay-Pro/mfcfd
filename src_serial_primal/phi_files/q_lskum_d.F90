...
!                *(point.temp):(loc) *(point.phi1):in *(point.phi2):in
!                *(point.delta):(loc)
...
!                point.temp:in point.phi1:in point.phi2:in point.delta:in
  SUBROUTINE Q_LSKUM_D()
    IMPLICIT NONE
    INTEGER :: i
    EXTERNAL UPDATE_BEGIN_PHI1_GHOST
    EXTERNAL UPDATE_END_PHI1_GHOST
    EXTERNAL UPDATE_BEGIN_PHI2_GHOST
    EXTERNAL UPDATE_END_PHI2_GHOST
    EXTERNAL IEEE_IS_NAN
    LOGICAL :: IEEE_IS_NAN
    INTEGER :: rank
    IF (rank .EQ. 0) OPEN(unit=301, file='residue', form='FORMATTED', status='REPLACE', action='WRITE') 
...
! Set U_old to U for first iteration
    DO i=1,local_points
      ...
      point%u_old(4, i) = 2.5d0*point%prim(4, i) + 0.5d0*point%prim(1, i)*(point%prim(2, i)*point%prim(2, i)+point%prim(3, i)*point%prim(3, i))
    END DO
    IF (rank .EQ. 0) THEN
