...
!                point.ddq:in point.phi1:in point.phi2:in point.delta:in
  SUBROUTINE CAL_FLUX_RESIDUAL_D()
    ...
    DO i=1,wall_points
      ...
      pointd%flux_res(:, k) = 2.0d0*(point%flux_res(:, k)*pointd%delta(k)+point%delta(k)*pointd%flux_res(:, k))
    ...
    DO i=1,outer_points
      ...
      pointd%flux_res(:, k) = (gxp+gxn+gyp)*pointd%delta(k) + point%delta(k)*(gxpd+gxnd+gypd)
    ...
    DO i=1,interior_points
      ...
      pointd%flux_res(:, k) = (gxp+gxn+gyp+gyn)*pointd%delta(k) + point%delta(k)*(gxpd+gxnd+gypd+gynd)
