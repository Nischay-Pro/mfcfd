...
!		
    ...
    INTRINSIC DSQRT
    REAL*8 :: arg1
    REAL*8, DIMENSION(ISIZE1OFtemp) :: temp0
    REAL*8 :: temp1
    REAL*8, DIMENSION(ISIZE1OFtemp) :: temp2
    REAL*8, DIMENSION(ISIZE1OFtemp) :: temp3
    REAL*8, DIMENSION(ISIZE1OFtemp) :: temp4
    sum_delx_sqr = 0.0d0
    sum_dely_sqr = 0.0d0
    sum_delx_dely = 0.0d0
    sum_delx_delf = 0.0d0
    ...
    DO j=1,point%xpos_nbhs(i)
      ...
      temp0 = delx*point%dq(1, :, i) + dely*point%dq(2, :, i)
      temp1 = 2.0d0*delx*dely
      temp2 = delx*delx*point%ddq(1, :, i) + temp1*point%ddq(2, :, i) + dely*dely*point%ddq(3, :, i)
      qtilde_id = pointd%q(:, i) - 0.5d0*(temp0*phi1_id+phi1_i*(delx*pointd%dq(1, :, i)+dely*pointd%dq(2, :, i))) + temp2*phi2_id/12d0 + phi2_i*(delx**2*pointd%ddq(1, :, i)+temp1*pointd%ddq(2, :, i)+dely**2*pointd%ddq(3, :, i))/12d0
      qtilde_i = point%q(:, i) - 0.5d0*(phi1_i*temp0) + phi2_i/12d0*temp2
      temp3 = delx*point%dq(1, :, k) + dely*point%dq(2, :, k)
      temp1 = 2.0d0*delx*dely
      temp4 = delx*delx*point%ddq(1, :, k) + temp1*point%ddq(2, :, k) + dely*dely*point%ddq(3, :, k)
      qtilde_kd = pointd%q(:, k) - 0.5d0*(temp3*phi1_kd+phi1_k*(delx*pointd%dq(1, :, k)+dely*pointd%dq(2, :, k))) + temp4*phi2_kd/12d0 + phi2_k*(delx**2*pointd%ddq(1, :, k)+temp1*pointd%ddq(2, :, k)+dely**2*pointd%ddq(3, :, k))/12d0
      qtilde_k = point%q(:, k) - 0.5d0*(phi1_k*temp3) + phi2_k/12d0*temp4
      CALL QTILDE_TO_PRIMITIVE_D(qtilde_i, qtilde_id, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL FLUX_GXP_D(g_i, g_id, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL QTILDE_TO_PRIMITIVE_D(qtilde_k, qtilde_kd, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL FLUX_GXP_D(g_k, g_kd, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
    ...
    one_by_det = 1.d0/det
    gd = one_by_det*(sum_dely_sqr*sum_delx_delfd-sum_delx_dely*sum_dely_delfd)
...
!		
    ...
    DO j=1,point%xpos_nbhs(i)
      ...
      sum_dely_sqr = sum_dely_sqr + deln*deln_weights
      sum_delx_dely = sum_delx_dely + dels*deln_weights
      qtilde_i = point%q(:, i) - 0.5d0*phi1_i*(delx*point%dq(1, :, i)+dely*point%dq(2, :, i)) + 1/12d0*phi2_i*(delx*delx*point%ddq(1, :, i)+2.0d0*delx*dely*point%ddq(2, :, i)+dely*dely*point%ddq(3, :, i))
      qtilde_k = point%q(:, k) - 0.5d0*phi1_k*(delx*point%dq(1, :, k)+dely*point%dq(2, :, k)) + 1/12d0*phi2_k*(delx*delx*point%ddq(1, :, k)+2.0d0*delx*dely*point%ddq(2, :, k)+dely*dely*point%ddq(3, :, k))
...
!  Hint: ISIZE1OFtemp should be the size of dimension 1 of array temp
    ...
    DO j=1,point%xneg_nbhs(i)
      ...
      qtilde_k = point%q(:, k) - 0.5d0*(phi1_k*temp3) + phi2_k/12d0*temp4
      CALL QTILDE_TO_PRIMITIVE_D(qtilde_i, qtilde_id, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL FLUX_GXN_D(g_i, g_id, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL QTILDE_TO_PRIMITIVE_D(qtilde_k, qtilde_kd, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL FLUX_GXN_D(g_k, g_kd, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      sum_delx_delfd = sum_delx_delfd + dels_weights*(g_kd-g_id)
      sum_delx_delf = sum_delx_delf + (g_k-g_i)*dels_weights
      sum_dely_delfd = sum_dely_delfd + deln_weights*(g_kd-g_id)
      sum_dely_delf = sum_dely_delf + (g_k-g_i)*deln_weights
...
!  Hint: ISIZE1OFtemp should be the size of dimension 1 of array temp
    ...
    DO j=1,point%ypos_nbhs(i)
      ...
      qtilde_k = point%q(:, k) - 0.5d0*(phi1_k*temp3) + phi2_k/12d0*temp4
      CALL QTILDE_TO_PRIMITIVE_D(qtilde_i, qtilde_id, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL FLUX_GYP_D(g_i, g_id, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL QTILDE_TO_PRIMITIVE_D(qtilde_k, qtilde_kd, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL FLUX_GYP_D(g_k, g_kd, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      sum_delx_delfd = sum_delx_delfd + dels_weights*(g_kd-g_id)
      sum_delx_delf = sum_delx_delf + (g_k-g_i)*dels_weights
      sum_dely_delfd = sum_dely_delfd + deln_weights*(g_kd-g_id)
      sum_dely_delf = sum_dely_delf + (g_k-g_i)*deln_weights
    ...
    det = sum_delx_sqr*sum_dely_sqr - sum_delx_dely*sum_delx_dely
    one_by_det = 1.d0/det
    gd = one_by_det*(sum_delx_sqr*sum_dely_delfd-sum_delx_dely*sum_delx_delfd)
...
!  Hint: ISIZE1OFtemp should be the size of dimension 1 of array temp
    ...
    DO j=1,point%yneg_nbhs(i)
      ...
      qtilde_k = point%q(:, k) - 0.5d0*(phi1_k*temp3) + phi2_k/12d0*temp4
      CALL QTILDE_TO_PRIMITIVE_D(qtilde_i, qtilde_id, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL FLUX_GYN_D(g_i, g_id, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL QTILDE_TO_PRIMITIVE_D(qtilde_k, qtilde_kd, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      CALL FLUX_GYN_D(g_k, g_kd, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
      sum_delx_delfd = sum_delx_delfd + dels_weights*(g_kd-g_id)
      sum_delx_delf = sum_delx_delf + (g_k-g_i)*dels_weights
      sum_dely_delfd = sum_dely_delfd + deln_weights*(g_kd-g_id)
      sum_dely_delf = sum_dely_delf + (g_k-g_i)*deln_weights
