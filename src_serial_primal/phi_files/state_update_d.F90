...
!                point.flux_res:in
  SUBROUTINE STATE_UPDATE_D(rk)
    ...
    DO i=1,wall_points
      ...
      nx = point%nx(k)
      ny = point%ny(k)
      CALL PRIMITIVE_TO_CONSERVED_D(point%prim(:, k), pointd%prim(:, k), nx, ny, u, ud)
      CALL PRIMITIVE_TO_CONSERVED_D(point%prim_old(:, k), pointd%prim_old(:, k), nx, ny, u_old, u_oldd)
      temp = u(1)
      IF (rk .NE. 3) THEN
      ...
      pointd%prim(4, k) = 0.4d0*(ud(4)-0.5d0*(temp0*tempd+temp*(2*u(2)*ud(2)+2*u(3)*ud(3))))
    ...
    DO i=1,outer_points
      ...
      nx = point%nx(k)
      ny = point%ny(k)
      CALL CONSERVED_VECTOR_UBAR_D(point%prim(:, k), pointd%prim(:, k), u, ud, nx, ny)
      CALL CONSERVED_VECTOR_UBAR_D(point%prim_old(:, k), pointd%prim_old(:, k), u_old, u_oldd, nx, ny)
      temp = u(1)
      IF (rk .NE. 3) THEN
        ud = ud - euler*0.5d0*pointd%flux_res(:, k)
        u = u - 0.5d0*euler*point%flux_res(:, k)
...
!   with respect to varying inputs: prim u
  SUBROUTINE PRIMITIVE_TO_CONSERVED_D(prim, primd, nx, ny, u, ud)
    ...
    ud(4) = 2.5d0*primd(4) + 0.5d0*(2*temp1*temp1d+2*temp2*temp2d-temp*rhod)/rho
...
! PetscErrorCode :: ierr
    DO i=1,local_points
      ...
      DO r=1,point%nbhs(i)
        ...
        delta_td = -(temp0*(mod_ud+3.0d0*result1d)/(mod_u+3.0d0*result1))
        delta_t = temp0
...
!   with respect to varying inputs: prim ubar
  SUBROUTINE CONSERVED_VECTOR_UBAR_D(prim, primd, ubar, ubard, nx, ny)
    ...
    ed = (prd-temp*(gamma-1.0d0)*rhod)/((gamma-1.0d0)*rho) + 0.5d0*temp1d
    e = temp + 0.5d0*temp1
    temp = rho/(2.0d0*pr)
    ...
    IF (beta .EQ. 0.0) THEN
      result11d = 0.0_8
    ELSE
      result11d = betad/(2.0*temp)
    END IF
    result11 = temp
    s2d = result11*u2_rotd + u2_rot*result11d
    s2 = u2_rot*result11
    arg1d = pi*betad
    arg1 = pi*beta
    temp = SQRT(arg1)
    IF (arg1 .EQ. 0.0) THEN
      result11d = 0.0_8
    ELSE
      result11d = arg1d/(2.0*temp)
    END IF
    result11 = temp
    temp = EXP(-(s2*s2))/(2.0d0*result11)
    b2d = (-(EXP(-(s2**2))*2*s2*s2d)-temp*2.0d0*result11d)/(2.0d0*result11)
    b2 = temp
    result10d = DERF_D(s2, s2d, result10)
    a2pd = 0.5d0*result10d
    a2p = 0.5d0*(1.0d0+result10)
    ubard(1) = a2p*rhod + rho*a2pd
    ubar(1) = rho_inf*a2n_inf + rho*a2p
    ubard(2) = a2p*(u1_rot*rhod+rho*u1_rotd) + rho*u1_rot*a2pd
    ...
    temp2d = e*(a2p*rhod+rho*a2pd) + rho*a2p*ed + 0.5d0*(b2*(u2_rot*rhod+rho*u2_rotd)+rho*u2_rot*b2d)
