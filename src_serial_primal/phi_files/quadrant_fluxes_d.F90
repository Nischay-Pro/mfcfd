...
!   with respect to varying inputs: g u1 u2 pr rho
  SUBROUTINE FLUX_QUAD_GXI_D(g, gd, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
    IMPLICIT NONE
...
!	Expressions for the split fluxes ..	
    gd(1) = (ut*a1neg-b1)*(a2neg*rhod+rho*a2negd) + rho*a2neg*(a1neg*utd+ut*a1negd-b1d)
    ...
    temp1d = a1neg*utd + ut*a1negd - b1d
    temp1 = ut*a1neg - b1
    temp4d = 0.5d0*(b2*temp1*(un*rhod+rho*und)+rho*un*(temp1*b2d+b2*temp1d))
    temp4 = 0.5d0*rho*un*b2*temp1
    gd(4) = (temp2-temp3)*(a2neg*rhod+rho*a2negd) + rho*a2neg*(temp2d-temp3d) - temp4d
...
!   variations   of useful results: g
!   with respect to varying inputs: g u1 u2 pr rho
  SUBROUTINE FLUX_QUAD_GXII_D(g, gd, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
    IMPLICIT NONE
    DOUBLE PRECISION :: g(4), u1, u2, rho, pr
    DOUBLE PRECISION :: gd(4), u1d, u2d, rhod, prd
    DOUBLE PRECISION :: tx, ty, nx, ny, ut, un
    ...
    result10d = DERF_D(s2, s2d, result10)
    a2negd = -(0.5d0*result10d)
    a2neg = 0.5d0*(1.d0-result10)
    pr_by_rhod = (prd-pr*rhod/rho)/rho
    pr_by_rho = pr/rho
    u_sqrd = 2*ut*utd + 2*un*und
    u_sqr = ut*ut + un*un
!	Expressions for the split fluxes ..	
    gd(1) = (ut*a1pos+b1)*(a2neg*rhod+rho*a2negd) + rho*a2neg*(a1pos*utd+ut*a1posd+b1d)
...
!   variations   of useful results: g
!   with respect to varying inputs: g u1 u2 pr rho
  SUBROUTINE FLUX_QUAD_GXIII_D(g, gd, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
    IMPLICIT NONE
    DOUBLE PRECISION :: g(4), u1, u2, rho, pr
    DOUBLE PRECISION :: gd(4), u1d, u2d, rhod, prd
    DOUBLE PRECISION :: tx, ty, nx, ny, ut, un
    ...
    u_sqr = ut*ut + un*un
!	Expressions for the split fluxes ..	
    gd(1) = (ut*a1pos+b1)*(a2pos*rhod+rho*a2posd) + rho*a2pos*(a1pos*utd+ut*a1posd+b1d)
    ...
    temp4d = 0.5d0*(b2*temp1*(un*rhod+rho*und)+rho*un*(temp1*b2d+b2*temp1d))
    temp4 = 0.5d0*rho*un*b2*temp1
    gd(4) = (temp2+temp3)*(a2pos*rhod+rho*a2posd) + rho*a2pos*(temp2d+temp3d) + temp4d
...
!   variations   of useful results: g
!   with respect to varying inputs: g u1 u2 pr rho
  SUBROUTINE FLUX_QUAD_GXIV_D(g, gd, nx, ny, u1, u1d, u2, u2d, rho, rhod, pr, prd)
    IMPLICIT NONE
    DOUBLE PRECISION :: g(4), u1, u2, rho, pr
    DOUBLE PRECISION :: gd(4), u1d, u2d, rhod, prd
    DOUBLE PRECISION :: tx, ty, nx, ny, ut, un
    ...
    u_sqr = ut*ut + un*un
!	Expressions for the split fluxes ..	
    gd(1) = (ut*a1neg-b1)*(a2pos*rhod+rho*a2posd) + rho*a2pos*(a1neg*utd+ut*a1negd-b1d)
