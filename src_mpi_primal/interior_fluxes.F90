...
!		
		...
		real*8 :: maxi(4), mini(4)
                ...
                do j = 1, point%xpos_nbhs(i)
                        ...
                        &(1/12d0)*phi2_i*(delx*delx*point%ddq(1,:,i) + 2.0d0*delx*dely*point%ddq(2,:,i) + dely*dely*point%ddq(3,:,i))
                        qtilde_k = point%q(:,k) - 0.5d0*phi1_k*(delx*point%dq(1,:,k) + dely*point%dq(2,:,k)) + &
                        &(1/12d0)*phi2_k*(delx*delx*point%ddq(1,:,k) + 2.0d0*delx*dely*point%ddq(2,:,k) + dely*dely*point%ddq(3,:,k))
